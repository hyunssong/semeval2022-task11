...
class DataTrainingArguments:
    ...
    line_by_line: bool = field(
        default=True, #each line is counted as a distinct sequence (the training data is consisted of short sentences )
...
def main():
        ...
        def tokenize_function(examples):
            ...
            for i in range(len(result["input_ids"])):
              ...
              if padding is "max_length":
                ...
                special_token_mask = [1]+special_token_mask
                #set attention mask value at 1 where sep exists
                sep_idx = result["input_ids"][i].index(3)
                special_token_mask[sep_idx]=1
                result["special_tokens_mask"].append(special_token_mask)
              else:
                for _ in range(len(result["input_ids"][i])):
                  special_token_mask.append(0)
            ...
            if padding is "max_length":
              assert len(result["special_tokens_mask"][0])==max_seq_length

            #sanity check
            assert len(result["special_tokens_mask"])==len(result["input_ids"])
            assert len(result["special_tokens_mask"][0]) == len(result["input_ids"][0])
            return result

            
            
            # for idx in range(len(result["special_tokens_mask"])):
            #   # print(result["special_tokens_mask"][idx])
            #   # print(result["input_ids"][idx])
            #   assert len(result["special_tokens_mask"][idx]) == len(result["input_ids"][idx])
            # print(padding)
    ...
    with training_args.strategy.scope():
        ...
        if checkpoint is not None:
            ...
            model = TFAutoModelForMaskedLM.from_pretrained(model_args.model_name_or_path, config=config,from_pt=True)
        else:
